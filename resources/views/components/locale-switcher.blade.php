@props(['class' => '', 'align' => 'right', 'labeled' => false])

@php
    $current = app()->getLocale();
    $currentLabel = $current === 'ar' ? __('diwan.locale.ar') : __('diwan.locale.en');
@endphp

<x-dropdown :align="$align" width="w-40" contentClasses="py-1 bg-white">
    <x-slot name="trigger">
        <button type="button"
                {{ $attributes->merge(['class' => ($labeled
                    ? 'inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 '
                    : 'inline-flex items-center justify-center rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 '
                ).$class]) }}
                aria-label="{{ __('diwan.locale.switch') }}"
                title="{{ __('diwan.locale.switch') }}">
            <svg class="{{ $labeled ? 'h-4 w-4 shrink-0 text-indigo-600' : 'h-5 w-5' }}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
            ...
            @if ($labeled)
                <span>{{ __('diwan.locale.switch') }}</span>
                <span class="rounded-md bg-indigo-100 px-1.5 py-0.5 text-xs font-semibold text-indigo-700">{{ $currentLabel }}</span>
